...
class Environment:  #定义一个名为Environment的类，用于模拟视频播放环境。
    ...
    def reset(self, user_sample_id, all_cooked_time, all_cooked_4gbw,all_cooked_wifibw,paths, video_num, seeds):  #我添加的reset函数。
        ...
        end_of_video=0
        first_step=1   
        result_array = np.array([delay,video_size,end_of_video,self.play_video_id,len(self.players),first_step,
    ...
    def play_videos(self, action_time):  #定义一个函数，用于播放视频,action_time也就是step函数中的delay，下载花了多长时间与此同时也就播放了多少时间。
        wasted_bw = 0
        buffer = 0
        ...
        while buffer >= 0 and action_time > 0:  #当缓冲区不为负且还有剩余播放时间时，继续播放。
            timeline_before_play = self.players[0].play_timeline       #获取当前视频的播放时间线。 players列表下标总是0-4
            video_remain_time = self.user_models[0].get_ret_duration() - timeline_before_play   #计算当前视频的剩余可播放时间。
            ...
            timeline_after_play, buffer = self.players[0].video_play(max_play_time)   #播放视频并更新播放后的时间线和缓冲区大小。
            ...
            if actual_play_time == video_remain_time:   #如果实际播放时间等于视频的剩余可播放时间，代表此视频播放完毕。
                ...
                print("User watched for ", self.user_models[0].get_ret_duration(), " ms, you downloaded ", self.players[0].get_chunk_counter()*1000, " ms.")   
                # 输出观看块的比特率和浪费带宽
                video_qualities = []   #定义视频质量列表，用于保存该视频每个块选择的比特率
                bitrate_cnt = min(math.ceil(self.players[0].get_play_chunk()), self.players[0].get_chunk_sum()) #用户播放了多少个块
                for i in range(1, bitrate_cnt):  #遍历下载的视频块。
                    video_qualities.append(self.players[0].get_video_quality(i-1))  #添加视频质量到列表。             
                ...
                wasted_bw += self.players[0].bandwidth_waste(self.user_models[0]) #计算浪费的带宽，只有当用户退出一个视频才会计算整个视频的浪费带宽。
            ...
            if self.play_video_id >= self.video_num:   #如果播放完所有7个视频。
                self.all_videos_done=1   #最后一个视频也结束
    ...
    def step(self, download_video_id, paths):  #原先的函数名是buffer_management，包含三个形参：下载的视频，比特率和睡眠时间
        buffer = 0  #当前缓冲区的时间长度。
        rebuf = 0   #重新缓冲的时间。
        end_of_video = False   #布尔值，表示当前播放的视频是否结束。
        delay = 0    #下载视频时的延迟。
        video_size = 0    #上个下载块的大小。       
        print("Download Video ", download_video_id, " chunk (", self.players[download_video_id].get_chunk_counter() + 1, " / ",
              self.players[download_video_id].get_chunk_sum(), ") and path is",paths,file=self.log_file)             
        ...
        buffer, wasted = self.play_videos(delay)  #调用play_videos函数模拟视频播放。 把delay传入action_time,意味着在下载视频块期间，播放器也会播放相应时间长度的视频内容。
        self.total_downloaded_len += 1000  # 更新总下载长度，加上1000毫秒。
        if download_video_id < self.start_video_id:  #如果下载的视频ID小于起始视频ID，表示用户滑动到了下一个视频。
            print("Extra chunk downloaded for Video ", download_video_id," which the user already finished watching.\n")
            wasted += video_size  # 已经流畅的播放完毕了，所以此时下载的块也是多余的，算进浪费变量
            end_of_video = True   #当前视频因用户滑动已结束               
        else:  #否则用户还在正常下载和播放
            if self.play_video_id == self.video_num:  # if user leaves  用户看完最后一个视频。
                self.all_videos_done=1                
            else:
                end_of_video = self.players[download_video_id-self.start_video_id].video_download(1000) #当前视频因所有的块下载完毕而结束。
        self.wasted_bytes += wasted   #更新总浪费的数据量。 
        if end_of_video==True:  
            endofvideo=1
        else:
            endofvideo=0  #当前视频结束标志
        first_step=0     
        ...
        s_prime = np.array([delay,video_size,endofvideo,self.play_video_id,len(self.players), first_step,
                                 self.players[0].get_buffer_size()/1000,self.players[1].get_buffer_size()/1000,self.players[2].get_buffer_size()/1000,self.players[3].get_buffer_size()/1000,self.players[4].get_buffer_size()/1000,
                                 self.players[0].get_remain_video_num(),self.players[1].get_remain_video_num(),self.players[2].get_remain_video_num(),self.players[3].get_remain_video_num(),self.players[4].get_remain_video_num()], dtype=np.float32)
